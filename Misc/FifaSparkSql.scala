...
object FifaSparkSql {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val wcplayers_df = spark.read
      ...
      .option("header", "true") //reading the headers
      .option("mode", "DROPMALFORMED")
      .load("src/main/resources/WorldCupPlayers.csv")
    ...
    val wcmatches_df = spark.read
      ...
      .option("header", "true") //reading the headers
      .option("mode", "DROPMALFORMED")
      .load("src/main/resources/WorldCupMatches.csv")
    ...
    // Find the Countries by years using WorldCup view

    val wcAtd = spark.sql("select Country,Year from WorldCup Order By Year")

    wcAtd.show()

    //Find the Winner by years using WorldCup view
    val wcgoal = spark.sql("select Winner,Year from WorldCup Order By Year")

    wcgoal.show()

    //HomeTeamGoals grouped on view wcMatches

    val cityCount = spark.sql("select Count(HomeTeamGoals),HomeTeamGoals from wcMatches Group By HomeTeamGoals")
    ...
    val match2013 = spark.sql("select count(*) from wcMatches where year=2013")
    ...
    val csv = sc.textFile("src/main/resources/WorldCups.csv")
